...
acc_models = {acc_model2, acc_model3, acc_model4, acc_model7, acc_model8 };

for acc_ind = 1:length(acc_models),
   
    GAMcluster_pred(acc_models{acc_ind}, timePeriod, main_dir,  'numFolds_lambda', 1, 'overwrite', false, 'lambda', .5);
...
dlpfc_model9 = 'Rule + Switch + Previous Error + Test Stimulus + Response Direction';
dlpfc_model10 = 'Rule + Test Stimulus + Response Direction';

dlpfc_models = {dlpfc_model9, dlpfc_model10, dlpfc_model3, dlpfc_model8};
% dlpfc_models = {dlpfc_model2};

for dlpfc_ind = 1:length(dlpfc_models),
   
    GAMcluster_pred(dlpfc_models{dlpfc_ind}, timePeriod, main_dir, 'numFolds_lambda', 1, 'overwrite', false, 'lambda', .5);
