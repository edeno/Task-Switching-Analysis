...
%% Parameter Estimates - Rule Interactions
...
stat = {pct_change(mult_change(par_sim)), pct_change(mult_change(abs(par_sim)))};
stat_names = {'Percent Change', 'Abs. Percent Change'};
ref_line = [0, 0];
xlims = {[-10 10], [-0.1 40]};

for stat_ind = 1:length(stat),
    cur_cov_level_names = nonInteraction_level_names(ismember(nonInteraction_cov_names, 'Rule'));
    pat = '(Rule:)|(:Rule)';
...
pct_ratio = @(level1, level2) pct_change(mult_ratio(level1, level2));
abs_pct_ratio = @(level1, level2) pct_change(abs_mult_ratio(level1, level2));
...
for cov_ind = 1:length(unique_cov_names),
    ...
    stat = {pct_ratio(cur_orient_inter_names, cur_color_inter_names), abs_pct_ratio(cur_orient_inter_names, cur_color_inter_names)};
    stat_names = {'Percent Change', 'Abs. Percent Change'};
