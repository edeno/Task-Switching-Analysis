function plotCorrelateAvgNeuronsToBehavior(modelName, timePeriod, varargin)
...
for session_ind = 1:length(sessionNames),
    ...
    parEstNeuron = cat(1, parEstNeuron, [mean(parEstN(strcmp(nBrainAreas, 'ACC'), 2:end), 1); mean(parEstN(strcmp(nBrainAreas, 'dlPFC'), 2:end), 1)]);
...
parEstNeuron = reshape(parEstNeuron, 2, length(sessionNames), size(parEstCorrect, 2));
...
for area_ind = 1:length(brainAreas),
    ...
    for plot_ind = 1:length(levelNames),
        ...
        plot(parEstCorrect(:, plot_ind), squeeze(parEstNeuron(area_ind, :, plot_ind)), '.', 'MarkerSize', 15)
