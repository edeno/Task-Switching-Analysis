function computeAPC_ruleswitch(regressionModel_str, timePeriod, main_dir, varargin)
...
save_dir = sprintf('%s/%s/Models/%s/APC/RuleSwitch/', data_info.processed_dir, timePeriod, regressionModel_str);
...
for session_ind = 1:length(session_names),
    ...
    createTask(apcJob{session_ind}, @avrPredComp_RuleSwitch, 0, {session_names{session_ind}, ...
