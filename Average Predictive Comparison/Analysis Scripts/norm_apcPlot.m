function norm_apcPlot(main_dir, timePeriod, model_name, covariate_type, varargin)
...
for neuron_ind = 1:numNeurons,
   apc(:, :, neuron_ind) = apc(:, :, neuron_ind)/baseline_firing(neuron_ind); 
end

abs_apc = [avpred.abs_apc];
abs_apc = reshape(abs_apc, [numLevels numSim numNeurons]);
for neuron_ind = 1:numNeurons,
   abs_apc(:, :, neuron_ind) = abs_apc(:, :, neuron_ind)/baseline_firing(neuron_ind); 
end

rms_apc = [avpred.rms_apc];
rms_apc = reshape(rms_apc, [numLevels numSim numNeurons]);
for neuron_ind = 1:numNeurons,
   rms_apc(:, :, neuron_ind) = rms_apc(:, :, neuron_ind)/baseline_firing(neuron_ind); 
end
...
stat = {apc, abs_apc};
stat_name = {'Norm. APC', 'Norm. ABS APC'};
...
for stat_ind = 1:length(stat),
    ...
    if strcmp(stat_name{stat_ind}, 'Norm. ABS APC'),
        ylim([0 .5]);
    else
        ylim([-0.5 0.5]);
    end
    ylabel('Norm. Spikes / s');
