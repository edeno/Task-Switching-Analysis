...
function [avpred] = avrPredCompReact(model, factorOfInterest, varargin)
...
saveFileName = sprintf('%s/%s_React_APC.mat', saveFolder, apcParams.factorOfInterest);
...
[beta, ~, stat] = glmfit(designMatrix, reactionTime, 'normal', 'link', 'log', 'constant', 'off');
...
for history_ind = 1:numHistoryFactors,
    ...
    for level_ind = 1:numLevels,
        ...
        for sim_ind = 1:apcParams.numSim,
            ...
            curLevelEst = exp(cLDM.Value * squeeze(parEst(:, sim_ind)));
            baselineLevelEst = exp(bDM.Value * squeeze(parEst(:, sim_ind)));
